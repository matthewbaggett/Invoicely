The following files are simply duplicates from the invoicely library. With
modifications in were to reference the imports. In theory, you could replace
them if you update the invoicely framework by copying them from that framework
again. Just make sure to modify the import paths to reference the
`./invoicely/less` folder:
```
./less/invoicely.less
...
your time customizing the various invoicely settings. Feel free to manually
edit it or even replace it with a service like
[BootTheme](http://www.boottheme.com).

The `./less/fixes.less` file contains various invoicely and Drupal fixes. It
